...
export default function Dashboard({ data, onReset, sessionId, googleEmail, user, onLogout, onAdmin, featureFlags = {} }) {
  ...
  // Auto-load Core Web Vitals after all state declared
  React.useEffect(() => {
    let cancelled = false
    if (url) {
      ...
      fetch('https://sem-ai-production.up.railway.app/api/pagespeed', {
        ...
        if (!cancelled) { setPageSpeed(d); setLoadingSpeed(false) }
      }).catch(() => { if (!cancelled) setLoadingSpeed(false) })
    }
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem' }}>
              <Card>
                ...
                {(() => {
                  ...
                  const seoItems = [
                    { label: 'Title Tag', score: breakdown.title_tag || breakdown.title_optimisation || titleScore, tip: `Title: "\${title.slice(0,40)}..." (\${title.length} chars). Ideal: 30-60 chars` },
                    { label: 'Meta Description', score: (breakdown.meta_description || breakdown.meta_descriptions) ?? metaScore, tip: `Meta: \${meta.length} chars. Ideal: 120-160 chars\${!meta ? ' — MISSING' : ''}` },
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              ...
              <Card>
                ...
                {(() => {
                  ...
                  const items = [
                    { label: 'Title Tag', score: breakdown.title_tag || breakdown.title_optimisation || (title.length >= 30 && title.length <= 60 ? 95 : title.length > 0 ? 60 : 0), detail: title ? title.length + ' chars' : 'Missing' },
                    { label: 'Meta Description', score: (breakdown.meta_description || breakdown.meta_descriptions) ?? (meta.length >= 120 && meta.length <= 160 ? 95 : meta.length > 0 ? 50 : 0), detail: meta ? meta.length + ' chars' : 'Missing' },
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          {recommended.map(s => {
                            const generateSchema = (type) => {
                              ...
                              const title = sc?.title || ''
                              const schemas = {
                                Organization: `<script type="application/ld+json">\n{\n  "@context": "https://schema.org",\n  "@type": "Organization",\n  "name": "${title.split('|')[0].trim()}",\n  "url": "${domain}",\n  "logo": "${domain.endsWith('/') ? domain.slice(0,-1) : domain}/logo.png",\n  "sameAs": []\n}\n</script>`,
